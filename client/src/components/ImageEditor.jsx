    ...
    <div className="flex flex-col h-full p-4 md:p-5 space-y-4 pb-10 md:pb-5">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-end gap-3">
        <div className="w-full md:w-auto">
          ...
          <select disabled className="text-base md:text-sm w-full md:w-36 opacity-70 cursor-not-allowed">
      ...
      <div>
        ...
        <input
          ...
          maxLength={200}
        />
        <p className="text-sm text-text-muted mt-1 text-right">{description.length}/200</p>
      </div>

      {/* Image preview */}
      {url.trim() && (
        <div>
          <label className="block text-sm font-medium text-text-muted mb-1">Preview</label>
          <div className="border border-border rounded-lg p-3 bg-bg-panel">
            {imgError ? (
              <div className="flex flex-col items-center justify-center py-6 text-text-muted">
                <ImageOff size={32} className="mb-2" />
                <p className="text-sm">Could not load image from this URL</p>
              </div>
            ) : (
              <img
                src={url}
      ...
      <div className="flex items-center justify-between pt-2">
        {image?._draft ? (
          <button onClick={onCancel} className="text-base md:text-sm text-text-muted hover:text-brand-black">
            Cancel
          </button>
        ) : (
          <button onClick={() => onDelete(image)} className="text-base md:text-sm text-red-600 hover:text-red-700">
            Delete image
          </button>
        )}
        <button
          ...
          className="bg-brand-orange hover:bg-brand-orange-hover active:bg-brand-orange-active text-white text-base md:text-sm font-medium px-4 py-2 rounded-lg disabled:opacity-50"
        >
